...
def load_runtime_manifest(root: Path | None = None) -> Dict[str, Any]:
    ...
    if not path.exists():
        raise RuntimeError("Governance Atlas runtime manifest is missing at runtime_manifest.yaml.")
...
def load_frontend_build_manifest(root: Path | None = None) -> Dict[str, Any]:
    ...
    if not manifest_path.exists():
        raise RuntimeError(
            "Governance Atlas requires a packaged frontend build manifest at "
    ...
    try:
        ...
        raise RuntimeError(
            f"Governance Atlas frontend build manifest is not valid JSON: {exc}"
        ) from exc
...
def validate_frontend_bundle(root: Path | None = None) -> Dict[str, Any]:
    ...
    if missing:
        ...
        raise RuntimeError(
            "Governance Atlas requires a packaged React frontend bundle at "
    ...
    if not source_hash:
        raise RuntimeError(
            "Governance Atlas frontend build manifest is missing the required `sourceHash` proof."
        )

    current_hash = frontend_source_hash(repo_root)
    if source_hash != current_hash:
        raise RuntimeError(
            "Governance Atlas frontend bundle is stale: frontend source changed after the "
