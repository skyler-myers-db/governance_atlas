...
2. `_user_role_slug` happy path preserves the table's role. An email
   listed in `GOVHUB_ADMIN_EMAILS` does NOT get re-promoted to admin if
...
def _stub_identity_module() -> None:
    """Stub govhub.api.identity so runtime_app imports cleanly in tests."""
    if "govhub.api.identity" in sys.modules:
        return
    module = ModuleType("govhub.api.identity")
    ...
    sys.modules["govhub.api.identity"] = module
...
class UserRoleSlugTests(unittest.TestCase):
    ...
    def test_happy_path_preserves_demoted_reader_even_if_in_admin_emails(
        ...
        """Regression test: an email in GOVHUB_ADMIN_EMAILS that has a
