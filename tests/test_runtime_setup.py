...
class RuntimeSetupPayloadTests(unittest.TestCase):
    def test_live_runtime_payload_surfaces_read_only_setup_truth(self) -> None:
        ...
        self.assertEqual(payload["auth"]["mode"], "forwarded-user-header")
        ...
        self.assertEqual(workspace_access["mode"], "forwarded-user-header")
        ...
        self.assertEqual(workspace_access["gates"][2]["state"], "unknown")
        self.assertEqual(workspace_access["gates"][2]["blockedSurfaces"], ["Queries, usage, and workloads"])
        self.assertIn("remediation", workspace_access["gates"][3])
    ...
    def test_missing_identity_and_config_surface_degraded_and_unavailable_checks(self) -> None:
        ...
        workspace_access = payload["workspaceAccess"]
        self.assertEqual(workspace_access["mode"], "read-only-no-identity")
        ...
        self.assertIn("Lineage graph, preview, and drawer", workspace_access["blockedSurfaces"])
        self.assertIn("Queries, usage, and workloads", workspace_access["blockedSurfaces"])
        self.assertIn("Discovery and detail export", workspace_access["blockedSurfaces"])
        self.assertIn("Background work runner", workspace_access["blockedSurfaces"])
        self.assertIn("Classification recommendations", workspace_access["blockedSurfaces"])
        self.assertEqual(workspace_access["gates"][0]["key"], "governance_writes")
