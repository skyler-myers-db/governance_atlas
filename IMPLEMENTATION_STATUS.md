...
Last updated: 2026-05-02 during reopened `northstar/*` audit continuation

## Current Authoritative State - Reviewer Signoff Recorded

- All prior `northstar/*` visual, functional, truth/provenance, and process signoffs are superseded unless they are revalidated by the current route-scoped matrix and active ledgers in this section.
- The active source of truth is `docs/northstar_gap_analysis/full_page_audit.md`; it currently has `0` open gaps: Command Center `0`, Discover `0`, Stewardship `0`, Glossary `0`, CDE Registry `0`, Lineage Atlas `0`, Audit Evidence `0`, Control Center `0`, and Cross-Page Shared `0`. The reopened visual, truth/provenance, functional workflow-proof, and process rows are closed only by their structured evidence rows and reviewer lanes.
- The active control-level source of truth is `docs/northstar_gap_analysis/functional_control_audit.md`; it currently has `0` open controls: Command Center `0`, Discover `0`, Stewardship `0`, Glossary `0`, CDE Registry `0`, Lineage Atlas `0`, Audit Evidence `0`, Control Center `0`, and Cross-Page Shared `0`. Control coverage is still a separate gate from visual parity and live Databricks truth.
- `docs/northstar_gap_analysis/signoff_matrix.md` records route-scoped SIGNOFF for every required reviewer role: visual fidelity, product structure, truth/provenance, functional workflow, process, and feedback coverage. Those matrix rows are tied to exact current screenshots, live truth reports, functional reports, ledger rows, and zero route/shared open rows.
- Current local prototype-mock visual evidence is `docs/northstar_visual_qa/all-routes-shared-layout-v415-local`. It is the current side-by-side visual evidence directory named by `docs/northstar_gap_analysis/reference_manifest.json`; it is local prototype-mock evidence, not live Databricks proof.
- Current live Databricks truth evidence is `docs/northstar_visual_qa/live-thin-state-v416-databricks/live-thin-state-report.json` for all-route thin/unavailable-state preservation and `docs/northstar_visual_qa/lineage-truth-v419-databricks/lineage-truth-report.json` for current Lineage table, column, restricted-upstream, restricted-downstream, and hidden-focus truth. Both reports are pinned in `reference_manifest.json` for build `frontend-3ff1094cfbf2` and deployment `01f145d4dfdb1a55bcec553fc2f90b7d`.
- Current functional-control evidence remains the control ledger plus the allowlisted interaction reports it cites, including `docs/northstar_visual_qa/all-functional-v122-local/prototype-current-report.json`, `docs/northstar_visual_qa/shared-mutation-functional-v342-local/prototype-current-report.json`, `docs/northstar_visual_qa/responsive-ai-fab-v412-local/prototype-current-report.json`, and the allowlisted deployed functional reports for Discover, Stewardship, Glossary, CDE, Audit, and Lineage. The latest v416/v419 live reports are truth/build evidence, not a replacement for the control-level ledger.
