...
const DEEP_LINK_ASSET =
  process.env.GOVHUB_SMOKE_ASSET_FQN || "prod.silver.ap_self_assessed_tax_dist";
const CDP_URL = process.env.GOVHUB_CDP_URL || "http://127.0.0.1:9223";
const OUT_DIR = process.env.GOVHUB_SMOKE_OUT_DIR || "/tmp/govhub-deployed-smoke";
const SMOKE_TIMEOUT_MS = Number(process.env.GOVHUB_SMOKE_TIMEOUT_MS || 15000);
const LOCAL_BUILD_MANIFEST_PATH = new URL("../dist/govhub-build-manifest.json", import.meta.url);
...
if (!BASE_URL) {
  console.error(
    "Usage: node frontend/scripts/govhub_deployed_smoke.mjs <base-url>",
  );
  process.exit(1);
}
...
async function copyChromeProfile(profileName) {
  ...
  for (const sourcePath of itemsToCopy) {
    ...
    try {
      ...
      // Best-effort only. Missing auth artifacts surface during navigation.
    }
  }
}
...
async function loadLocalBuildManifest() {
  try {
    return JSON.parse(await fs.readFile(LOCAL_BUILD_MANIFEST_PATH, "utf-8"));
  } catch (error) {
    await pushCheck("local-build-manifest", "error", {
      path: LOCAL_BUILD_MANIFEST_PATH.pathname,
      message: error?.message || String(error),
    });
    throw error;
  }
