...
const OUT_DIR =
  process.env.GOVAT_QA_OUT_DIR ||
  process.env.GOVAT_QA_OUT_DIR ||
  path.join(REPO_ROOT, "docs", "screenshots", "identity");

if (!BASE_URL) {
  console.error(
    "Usage: node frontend/scripts/atlas_structural_identity.mjs <base-url>",
  );
  process.exit(1);
}
...
const traits = [
  // ─────── TOP BAR (items 1–6) ───────
  {
    ...
    name: "Top bar: Entrada brand mark",
    check: async (page) => {
      const mark = page.locator(".gh-shell-brand-mark img[alt='Entrada']");
      if (!(await mark.count())) return false;
      const src = await mark.first().getAttribute("src");
      return String(src || "").includes("entrada");
    },
  },
  {
